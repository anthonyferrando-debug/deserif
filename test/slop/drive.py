...
# 10. what an extension update leaves behind: a second injection while the
#     script is alive must be a no-op, and an orphaned instance must be retired
#     by the next one, which takes over without a tab reload.
popup.eval("chrome.storage.sync.set({disabledHosts: []})", aw=True)
st = wait_settled(seconds=6, min_age=1)
tid = popup.eval("chrome.tabs.query({url: 'https://www.facebook.com/*'}).then(ts => ts[0].id)", aw=True)
EXEC = "chrome.scripting.executeScript({target: {tabId: %d}, %s}).then(r => r && r[0] ? r[0].result : null)"
before = popup.eval(TAB % 'slop:getStats', aw=True)
popup.eval(EXEC % (tid, "files: ['slop.js']"), aw=True)
time.sleep(0.6)
marker = popup.eval(EXEC % (tid, "func: () => ({ hasApi: typeof window.__deserifSlop === 'object', alive: window.__deserifSlop.alive() })"), aw=True)
check(bool(marker and marker.get('hasApi') and marker.get('alive')), 'live instance exposes alive(): %s' % json.dumps(marker))
after = popup.eval(TAB % 'slop:getStats', aw=True)
check(after == before, 'second injection while alive is a no-op: %s' % json.dumps(after))
popup.eval(EXEC % (tid, "func: () => { const old = window.__deserifSlop; window.__deserifSlop = { alive: () => false, retire: () => { window.__oldRetired = true; old.retire(); } }; return true; }"), aw=True)
popup.eval(EXEC % (tid, "files: ['slop.js']"), aw=True)
st = wait_settled(seconds=10, min_age=1.5)
retired = popup.eval(EXEC % (tid, "func: () => window.__oldRetired === true"), aw=True)
check(retired is True, 'the new instance retired the orphaned one')
check(all(st[k]['st'] == v for k, v in EXPECT.items()), 'takeover: slop hidden again without a tab reload (%s)' % {k: st[k]['st'] for k in EXPECT if st[k]['st'] != EXPECT[k]})
s2 = popup.eval(TAB % 'slop:getStats', aw=True)
blocked_list = page.eval("Array.from(document.querySelectorAll('img[data-deserif-slop=\"blocked\"]')).map(i => i.id || (i.className + ':' + (i.getAttribute('src') || '').slice(0, 40)))")
check(bool(s2 and s2['blocked'] == 10 and s2['units'] == 10 and s2['active']), 'new instance answers the popup: %s blocked=%s' % (json.dumps(s2), blocked_list))
pt = page.eval("(() => { document.getElementById('p_poster').scrollIntoView({block: 'center'}); const r = document.getElementById('p_poster').getBoundingClientRect(); return { x: r.x + r.width / 2, y: r.y + r.height / 2 }; })()")
page.call('Input.dispatchMouseEvent', type='mousePressed', x=pt['x'], y=pt['y'], button='left', clickCount=1)
page.call('Input.dispatchMouseEvent', type='mouseReleased', x=pt['x'], y=pt['y'], button='left', clickCount=1)
time.sleep(0.5)
st = state()
check(st['p_poster']['st'] == 'shown' and st['p_poster']['src'].startswith('https://scontent') and st['__hash'] == '', 'click still reveals after the takeover')
